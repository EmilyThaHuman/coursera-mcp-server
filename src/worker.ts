...
async function searchCoursesAPI(
  ...
  const RAPIDAPI_HOST = "coursera-course-scraper-api.p.rapidapi.com";

  if (!RAPIDAPI_KEY) {
    console.log("[worker.ts][searchCoursesAPI] --> RAPIDAPI_KEY not set, using mock data");
    return null;
  }
  ...
  try {
    ...
    console.log(`[worker.ts][searchCoursesAPI] --> Searching: ${searchUrl}?${searchParams}`);
    ...
    if (!response.ok) {
      console.warn(`[worker.ts][searchCoursesAPI] --> API error: ${response.status}`);
      return null;
    }
